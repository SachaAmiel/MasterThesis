...
Quantum Energy Inequalities are covariant lower bounds on the energy of a given physics. They do not tell us much about the physics itself, nor about the spacetime in which the physics unfolds; but from the restrictions they impose on energy, one can (if following general relativity) impose restrictions on that spacetime. This can bring crucial insight in the field of quantum field theory in curved spaces, and therefore, potentially quantum gravity.
...
Now, back to the subject at hand, what are theories of physics we will take as our axioms? Those will be theory of \textit{General Relativity} (or ``GR") and that of \textit{Quantum Field Theory} (or ``QFT"). But already, two issues show up.
...
The second, and most problematic issue, is that QFT and GR are two significantly different theories, thus, one cannot candidly ``consider" the two all at once. We will thus be very cautious, when joining them to make the following distinction: we will study a QFT on spacetime first, and then possibly have that spacetime follow the laws of general relativity. To do so, we will need to generalise QFT in Minkowski space to QFT in curved spaces, and since we claim doing mathematical physics, we will need to justify that the way we do so is canonical in some sense, (as we do not want to be making a theory in any way different from what is currently accepted by accidentally adding theoretical behaviours). Getting such a ``natural" extension of QFT will be the focus of section \ref{1.2} .
...
Singularity theorems, given a particular bound on the energy distribution of a manifold, tell us that there exists singularities or event horizons on that manifold. In a way, they allow to ensure the existence of black holes without having to find them. If we were to have such energy bounds for a QFT, we would, consequently, be doing the same it the quantum case. This is what \textit{quantum energy inequalities with curvatures} are.

In section \ref{2}, we will (first, in section \ref{2.1}) be presenting the classical zoology of energy conditions. Indeed, as the reader is probably already aware, in general relativity, energy is a tensor $T^{\mu\nu}$ and not a scalar, while an energy density, will be a scalar field. Thus, one needs to \textit{contract} that tensor, and as there are multiple ways of doing so, there is a reach zoology of energy conditions, which section \ref{2.1} will only scratch the surface of.
...
In section \ref{2.2}, we will thus present them, with their different formalism, and we will also illustrate the presentation with the computation of a simple example (``simple" because in the flat spacetime case and using a vacuum state as a point of comparison) before focusing on our attempt to compute a more general one (curved space and no comparison) in the rest of the text. 

Prior to our work, there have been several successful attempts at computing many different types of quantum energy inequalities. A complete review of the field can be found in \cite{EleRev} or in \cite{FewLect}. In recent advances, the time-like case was extensively studied by \cite{OtherOlum}, \cite{TimeLikeCompute} and \cite{QStrong}. General bounds were also obtained in \cite{WolrdLine} and \cite{FourBound}. However, as far as the present project is concerned, we are going to follow the footsteps of \cite{DSNEC} which extensively computed the null case \textit{in flat space only} and called for a continuation of the work in order to get to a general null bound: the \textit{Quantum Double Smeared Null Energy Condition} in the curved spacetime case. This would be particularly powerful as the null case is a thinner bound then the time-like one.
...
In this section, we will use the symbol $\mathbb{M}_n$ to refer to Minkowski spacetime in $1+(n-1)$ dimensions, and $\mathcal{M}$ will denote any spacetime with signature\footnote{To be honest, this convention is useless in this section, where everything is very general (i.e. convention independent), but since we use it in section \ref{2}, one ought to assume it from now, for simplicity's sake.} $(+, -,-, ..., -)$. Any other\footnote{Other than standard undergrad ones (which we assume the reader is familiar with), and concepts related to the notion of foliation, (of which an introduction can also be found in the appendix, both in terms of physical interpretation in section \ref{PhyFoli} or mathematically rigorous notions in section \ref{MatFoli}).} notations and concepts will be introduced before being used.
...
\label{Com:SymDiracWeyl}
    ...
    All this, (associated to the fact that $\mathbb{M}_4$ can be foliated\footnote{If the reader is unfamiliar with the notion of foliation, it is introduced in the appendix. in section \ref{MatFoli} of the Mathematics appendix is a formal introduction, and a physical interpretation can be found in section \ref{PhyFoli} of the Physics Appendix.} with respect to time) sort of hints us towards the idea that AQFT is indeed sort of what a QFT should be (i.e. a covariant QM).
...
\begin{comment}
    In the case where any two such representations are unitarily equivalent, we see that the given QFT \textbf{\textit{is}} a quantum mechanics... But it turns out that many QFTs do not have this property, in particular the algebra of operators of free field theories do not have this property.
...
\end{equation}
But of course, we can also do $d=2$ using:\\
$\forall n \in \mathbb{N}^*$
\begin{equation}
...
\begin{thebibliography}{99}
    ...
    \bibitem{WolrdLine}
    Fewster, C.; \textit{A General Worldline Quantum Inequality}
\end{thebibliography} Classical and Quantum Gravity \href{https://iopscience.iop.org/article/10.1088/0264-9381/17/9/302}{17 1897-1911} (2000)
\end{document}