...
\begin{definition}[Wavefront Set]
    ...
    \begin{equation}
    \forall N \in \mathbb{N} \mathcal{FT}(\underset{k\in C}{\mathrm{sup}} (1+|k|)^N|\hat{f\cdot u})<\infty
    \end{equation}
    where $\mathcal{FT}$ denotes the Fourrier transform, though its definition on a manifold is chart-dependent, its boundedness isn't \color{red} The hat is not spreading also that bit has to go earlier\color{black}.
...
\begin{align}
    ...
    \notag\quad\quad\quad\quad\quad\quad\quad\quad\\
